...
def build_report_plain_lines(req: ExportRequest) -> List[str]:
    """导出依赖缺失时使用的纯文本内容，保持与正式报告相同的信息结构。"""
    is_zh = req.lang == "zh"
    ...
    add(title)
    add(f"{'问题' if is_zh else 'Question'}: {req.question}")
    add(f"{'模式' if is_zh else 'Mode'}: {mode_label}")
    add(f"{'日期' if is_zh else 'Date'}: {time.strftime('%Y-%m-%d %H:%M')}")
    add("")

    if req.steps:
        add("推理过程" if is_zh else "Reasoning Process")
        for step in req.steps:
            add(step.get("title", ""))
            add(step.get("content", ""))
            add("")

    if sim_data and req.mode == "simulation":
        add("多智能体推演过程" if is_zh else "Multi-Agent Simulation Process")
        for pm in sim_data.get("phaseMessages", []):
            add(pm.get("message", ""))
        for agent in sim_data.get("agents", []):
            add(f"{agent.get('name', '')} / {agent.get('role', '')} / {agent.get('stance', 'neutral')}")
            add(agent.get("expertise", ""))
            add(agent.get("persona", ""))
            add("")
        for rnd in sim_data.get("rounds", []):
            add(f"第{rnd.get('round', '?')}轮讨论" if is_zh else f"Round {rnd.get('round', '?')} Discussion")
            for speech in rnd.get("speeches", []):
                add(f"{speech.get('agent_name', '')}: {speech.get('content', '')}")
            if rnd.get("summary"):
                add(("轮次摘要: " if is_zh else "Round Summary: ") + str(rnd.get("summary", "")))
            add("")
        if sim_data.get("coordinatorSummary"):
            add("协调者总结" if is_zh else "Coordinator Summary")
            add(sim_data.get("coordinatorSummary", ""))
            add("")

    add("预测结论" if is_zh else "Prediction Conclusion")
    add(pred.get("conclusion", ""))
    confidence = int(float(pred.get("confidence", 0.7) or 0.7) * 100)
    add(f"{'置信度' if is_zh else 'Confidence'}: {confidence}%")
    add("")

    evidence = pred.get("evidence", []) or []
    if evidence:
        add("支撑证据" if is_zh else "Supporting Evidence")
    ...
    if key_factors:
        add("关键因素" if is_zh else "Key Factors")
...
def build_report_docx_fallback(req: ExportRequest) -> bytes:
    """不依赖 python-docx 的最小 DOCX 兜底生成器。"""
    import io
    import zipfile
    ...
    body = []
    lines = build_report_plain_lines(req)
    for idx, line in enumerate(lines):
        if idx == 0:
            body.append(p(line, "Title"))
        elif line in {"推理过程", "Reasoning Process", "多智能体推演过程", "Multi-Agent Simulation Process", "预测结论", "Prediction Conclusion", "支撑证据", "Supporting Evidence", "关键因素", "Key Factors", "相关文献列表", "Related Literature"}:
            body.append(p(line, "Heading1"))
        else:
            body.append(p(line))
...
def build_report_pdf_fallback(req: ExportRequest) -> bytes:
    """不依赖 ReportLab 的最小 PDF 兜底生成器。ReportLab 可用时不会走到这里。"""
    import textwrap

    raw_lines: List[str] = []
    for line in build_report_plain_lines(req):
        if not line:
            raw_lines.append("")
            continue
        width = 38 if any("\u4e00" <= ch <= "\u9fff" for ch in line) else 82
        raw_lines.extend(textwrap.wrap(line, width=width, replace_whitespace=False, drop_whitespace=False) or [""])

    pages = [raw_lines[i:i + 42] for i in range(0, len(raw_lines), 42)] or [[]]
    objects: List[bytes] = []
    ...
    for page in pages:
        ...
        for line in page:
            commands.append(f"{pdf_hex(line)} Tj")
            commands.append("T*")
        commands.append("ET")
        stream = "\n".join(commands).encode("ascii")
        content_id = add_obj(b"<< /Length " + str(len(stream)).encode("ascii") + b" >>\nstream\n" + stream + b"\nendstream")
        page_id = add_obj(f"<< /Type /Page /Parent 2 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 {font_id} 0 R >> >> /Contents {content_id} 0 R >>".encode("ascii"))
...
def build_report_docx(req: ExportRequest) -> bytes:
    ...
    try:
        ...
        print(f"[导出] python-docx 不可用，使用内置 DOCX 兜底: {import_error}")
        return build_report_docx_fallback(req)
...
def build_report_pdf(req: ExportRequest) -> bytes:
    ...
    try:
        ...
        print(f"[导出] ReportLab 不可用，使用内置 PDF 兜底: {import_error}")
        return build_report_pdf_fallback(req)
